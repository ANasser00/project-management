...
flowchart LR
  ...
  subgraph CLIENT["Client - Next.js and React"]
    UI[UI Pages and Components: Home, Projects, Tasks, Timeline, Chat]
    STATE[App State: Redux and RTK Query]
    AI_CHAT[AI Task Chat UI - AiTaskChatBox]
    AUTH_UI[Auth UI - Login and Register]
  end

  subgraph SERVER["Backend API - Express and Prisma"]
  ...
  %% Client <-> Server
  UI -->|HTTP - fetch and RTK Query| AUTH_API
  UI --> USER_API
  UI --> PROJECT_API
  UI --> TASK_API
  UI --> SEARCH_API
  ...
  %% AI flow
  AI_API -->|Prompt and context - tasks, users, projects| GEMINI
  GEMINI -->|Structured JSON - action and fields| AI_API
  AI_API --> AI_CHAT
